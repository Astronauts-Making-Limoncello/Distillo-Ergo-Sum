...
from TransUNet_Latent_Space_args import get_args
...
import TransUNet_Latent_Space_args as args
...
def _train(
    ...
    for epoch in range(starting_epoch, starting_epoch + args.num_epochs + 1):
        ...
        for batch_train in list(dl_train)[:num_batches_train]:
            ...
            outputs, outputs_latent = model(img_batch_train)
...
def _validate(
    ...
    for batch_val in list(dl_val)[: num_batches_val]:
        ...
        for ind in range(num_slices):
            ...
            with torch.no_grad():
                outputs, outputs_latent = model(input)
